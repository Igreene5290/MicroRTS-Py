...
def get_bot_name(bot, bot_type):
    if bot_type == "agent":
        ...
        return split_path[-2] if split_path[-1] == "" else split_path[-1]
    elif bot_type == "ai":
...
def print_final_results(bots_wins, all_bots):
    ...
    header = (
        " " * 15
        + " | "
        + " | ".join(f"{name:>15}" for name in bot_names)
        + " | Total Wins"
    )
    print(header)
    print("-" * len(header))
...
if __name__ == "__main__":
    ...
    for i in range(len(all_bots)):
        for j in range(i + 1, len(all_bots)):
            ...
            # determine mode and env counts
            if player1_type == "agent" and player2_type == "agent":
                bot_envs, selfplay_envs, mode = 0, 2, 0
            elif (
                (player1_type == "agent" and player2_type == "ai")
                or (player1_type == "ai" and player2_type == "agent")
            ):
                bot_envs, selfplay_envs, mode = 1, 0, 1
                if player1_type == "ai":
                ...
                bot_envs, selfplay_envs, mode = 1, 0, 2
                ai1s = [eval(f"microrts_ai.{all_bots[i]['classname']}")]
                ai2s = [eval(f"microrts_ai.{all_bots[j]['classname']}")]
            elif (
                (player1_type == "dt" and player2_type == "agent")
                or (player1_type == "agent" and player2_type == "dt")
            ):
                bot_envs, selfplay_envs, mode = 0, 2, 3
            elif (
                (player1_type == "dt" and player2_type == "ai")
                or (player1_type == "ai" and player2_type == "dt")
            ):
                bot_envs, selfplay_envs, mode = 1, 0, 4
                if player1_type == "ai":
                    ai2s = [eval(f"microrts_ai.{all_bots[i]['classname']}")]
                else:
                    ai2s = [eval(f"microrts_ai.{all_bots[j]['classname']}")]
            ...
            if mode == 0:
                ...
                agent1.load_state_dict(
                    torch.load(all_bots[i]["path"], map_location=device),
                    strict=False,
                )
                agent1.eval()
                agent2.load_state_dict(
                    torch.load(all_bots[j]["path"], map_location=device),
                    strict=False,
                )
                agent2.eval()
                ...
                agent1 = Agent(envs).to(device)
                load_path = all_bots[i]["path"] if player1_type == "agent" else all_bots[j]["path"]
                agent1.load_state_dict(torch.load(load_path, map_location=device), strict=False)
                agent1.eval()
                next_obs = torch.Tensor(envs.reset()).to(device)
            elif mode == 2:
                next_obs = envs.reset()
                ...
                # DT vs Agent/AI
                dt_path = all_bots[i]["path"] if player1_type == "dt" else all_bots[j]["path"]
                agent1 = TrainableDT.from_pretrained(dt_path).to(device)
                agent1.eval()
                if mode == 3:
                    # Agent vs DT: second player is standard Agent
                    agent2 = Agent(envs).to(device)
                    agent2.load_state_dict(
                        torch.load(
            ...
            for game in range(args.games_per_match):
                # initialize DT trajectory
                if mode in {3, 4}:
                    states = decode_obs(next_obs[0].view(mapsize, -1))\
                        .reshape(1, collector.state_dim)\
                ...
                for update in range(max_ep_length):
                    ...
                    if mode == 0:
                        with torch.no_grad():
                            ...
                            action = torch.zeros((bot_envs + selfplay_envs, p2_action.shape[1], p2_action.shape[2]))
                            action[::2] = p1_action
                        ...
                        with torch.no_grad():
                            ...
                            # DT action
                            actions = torch.cat([
                                actions, torch.zeros((1, collector.act_dim), device=device)
                            ], dim=0)
                            rewards = torch.cat([
                            ...
                            action = torch.zeros((num_envs, mapsize, 7), device=device)
                            ...
                            timesteps = torch.cat([
                                timesteps,
                                torch.ones((1, 1), device=device, dtype=torch.long) * (update + 1)
                            ], dim=1)
                    ...
                    for idx, info in enumerate(infos):
                        if "episode" in info.keys():
                            ...
                            if mode == 0:
                                ...
                                is_p1 = (player1_type == "agent" and mode == 1) or (player1_type == "dt" and mode == 4)
                                if score > 0:
                                    bots_wins[i][j] += int(is_p1)
                                    bots_wins[j][i] += int(not is_p1)
                                    print(f"Game {game+1}: {p1_name if is_p1 else p2_name} wins!")
                                else:
                                    print(f"Game {game+1}: it's a draw!")
                            elif mode == 2:
                                if score > 0:
