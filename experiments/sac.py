#!/usr/bin/env python3
"""
Implementation of Soft Actor-Critic (SAC) for MicroRTS,
based on the structure of ppo_gridnet.py and the GridNet paper
(see http://proceedings.mlr.press/v97/han19a/han19a.pdf).
"""
...
def parse_args():
    ...
    # SAC/algorithm-specific parameters
    ...
    parser.add_argument('--auto-alpha', type=lambda x: bool(strtobool(x)), default=True, nargs='?', const=True,
                        help='Automatic entropy tuning flag')
    ...
    parser.add_argument('--num-models', type=int, default=100,
                        help='Number of models (checkpoints) to save')
    parser.add_argument('--max-eval-workers', type=int, default=4,
                        help='Max evaluation workers (if using evaluation)')
    ...
    parser.add_argument('--anneal-lr', type=lambda x: bool(strtobool(x)), default=True, nargs='?', const=True,
                        help="Toggle learning rate annealing for SAC")
...
class ReplayBuffer:
    def __init__(self, buffer_size, observation_shape, action_shape, device):
        ...
        self.invalid_action_masks = np.zeros((buffer_size, action_shape[0] * action_shape[1]), dtype=np.float32)
        self.next_invalid_action_masks = np.zeros((buffer_size, action_shape[0] * action_shape[1]), dtype=np.float32)
        self.device = device
...
def run_training():
    ...
    # Define shapes.
    mapsize = 16 * 16
    action_shape = (mapsize, len(envs.action_plane_space.nvec))  # Use len(nvec) for discrete actions
    invalid_action_shape = (mapsize, int(np.sum(envs.action_plane_space.nvec)))
    rb = ReplayBuffer(args.buffer_size, envs.observation_space.shape, action_shape, device)


    # Patch environment (if necessary)
    # (Assuming previous patch logic is in place as needed.)
    
    agent = Agent(envs, device, mapsize)
    
    qf1_optimizer = optim.Adam(agent.qf1.parameters(), lr=args.learning_rate)
    qf2_optimizer = optim.Adam(agent.qf2.parameters(), lr=args.learning_rate)
    ...
    # (If using league evaluation, you can set that up here.)
    ...
    for update in range(1, num_updates + 1):
        ...
        if args.anneal_lr:
            ...
            for opt in [qf1_optimizer, qf2_optimizer, actor_optimizer]:
        ...
        for step in range(args.num_steps):
            ...
            for i in range(args.num_envs):
                rb.add(
                    ...
                    next_obs[i],
                    dones[i],
                    invalid_action_masks[i].cpu().numpy(),
                    np.array(envs.get_action_mask()[i])
                )
            obs = next_obs
            ...
            for _ in range(args.num_steps):
                ...
                with torch.no_grad():
                    next_state_actions, next_state_log_pi, _, _, _, _ = agent.get_action_and_value(
                        next_observations, next_iam.reshape(-1, invalid_action_shape[1])
                    )
                ...
                actor_actions, log_pi, _, _, _, _ = agent.get_action_and_value(observations, iam.reshape(-1, invalid_action_shape[1]))
                actor_q1 = agent.qf1(observations, actor_actions)
                actor_q2 = agent.qf2(observations, actor_actions)
                actor_q = torch.min(actor_q1, actor_q2)
                actor_loss = (alpha * log_pi - actor_q).mean()
        ...
        print(f"Update {update}: Global Steps = {global_step}, Critic Loss = {critic_loss.item():.4f}, Actor Loss = {actor_loss.item():.4f}")
        update += 1
    writer.close()
    envs.close()
